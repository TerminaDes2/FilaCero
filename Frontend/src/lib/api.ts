...
  // Normalize headers into a plain object to avoid issues when `init.headers` is
  // a Headers instance or an array of tuples. Keep Content-Type by default and
  // add Authorization when token is present.
...
      if (!negocioId) {
  const negocioEnv = (globalThis as any).process?.env?.NEXT_PUBLIC_NEGOCIO_ID;
...
      try { negocioId = typeof window !== 'undefined' ? localStorage.getItem('active_business_id') || undefined : undefined; } catch {}
  if (!negocioId) negocioId = (globalThis as any).process?.env?.NEXT_PUBLIC_NEGOCIO_ID as string | undefined;
      if (negocioId) merged.id_negocio = negocioId;
    }

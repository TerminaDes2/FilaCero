...
export const LoginForm: React.FC<LoginFormProps> = ({ onSuccess }) => {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Verificamos si el usuario tiene negocios asignados
            console.log('🔄 Verificando asignación de negocios...');
            const misNegocios = await api.listMyBusinesses();

            // 3. Redirigimos basándonos en si tiene negocios o no
            if (misNegocios && misNegocios.length > 0) {
                // ¡SÍ TIENE NEGOCIOS! (Este es el caso de 'mapupu')
                console.log('✅ Usuario asignado a un negocio. Redirigiendo a /pos');

                // Guardamos el primer negocio como el activo
                const primerNegocio = misNegocios[0];
                activeBusiness.set(String(primerNegocio.id_negocio));

                // Redirigimos al Panel del POS
                router.push('/pos');

            } else {
                // NO TIENE NEGOCIOS (Caso de un cliente nuevo)
                console.log('ℹ️ Usuario no asignado a ningún negocio. Redirigiendo a /shop');
                router.push('/shop');
            }

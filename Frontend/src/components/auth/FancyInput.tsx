...
export const FancyInput: React.FC<FancyInputProps> = ({
  ...
  return (
    <div className="relative group">
      <div className={`relative rounded-xl border bg-white/80 dark:bg-slate-800/60 backdrop-blur transition
        ${hasError ? '' : 'border-gray-300/60 dark:border-slate-600/60'}
        focus-within:ring-2 focus-within:ring-brand-400/30 focus-within:dark:ring-brand-300/25
      ...
      >
        ...
        <label
          ...
          className={`absolute ${leftIcon ? 'left-10' : 'left-3'} transition-[top,transform,color,background-color] px-1 rounded-md text-[11px] tracking-wide font-medium
            ${(focused || hasContent) ? '-top-2.5 scale-90 bg-white/95 dark:bg-slate-900/90 text-brand-600 dark:text-brand-300 shadow-sm' : 'top-3 text-gray-500 dark:text-slate-400'}
            pointer-events-none origin-left duration-200`}
        >
          {label}
        </label>
        {leftIcon && (
          <span className="absolute inset-y-0 left-3 flex items-center pt-0.5 text-gray-400 dark:text-slate-500 pointer-events-none">
            {leftIcon}
          </span>
        )}
        <input
          ...
          className={`w-full rounded-xl bg-transparent outline-none appearance-none text-sm text-gray-800 dark:text-slate-100 placeholder-transparent
        ...
        {(isPassword || rightIcon) && (
          <button
            ...
            className="absolute inset-y-0 right-3 flex items-center text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 transition select-none focus:outline-none focus:ring-0"
      ...
      {typeof strength === 'number' && (
        <div className="mt-1 flex gap-1" aria-hidden>
          {[0,1,2,3].map(i => {
            ...
            const color = !active ? 'bg-gray-300 dark:bg-slate-600' : strength <=1 ? 'bg-rose-400' : strength ===2 ? 'bg-amber-400' : strength ===3 ? 'bg-brand-500' : 'bg-green-500';
            return <span key={i} className={`h-1.5 flex-1 rounded-full transition-all ${color}`} />;
          })}
        </div>
      )}
      {hint && !error && (
        <p id={`${id}-hint`} className="mt-1 text-xs text-gray-500 dark:text-slate-400">{hint}</p>
      )}
      {hasError && (
        <p id={`${id}-error`} role="alert" className="mt-1 text-xs font-medium text-rose-600 dark:text-rose-400">{error}</p>
      )}
    </div>
  );
};

  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 animate-fadeIn dark:bg-slate-950/70">
      <div className="relative w-full max-w-md rounded-2xl bg-white p-8 shadow-xl animate-slideUp dark:bg-slate-900/90 dark:shadow-brand-950/40">
        ...
        <h2 className="mb-2 text-2xl font-semibold text-gray-900 dark:text-white">{plan.name}</h2>
        <div className="mb-4 text-3xl font-semibold text-gray-900 dark:text-white">
          {plan.price}{" "}
          {plan.period && (
        ...
        <ul className="group mb-6 space-y-2 text-gray-700 dark:text-slate-200">
          {plan.features.map((f) => (
            <li
              key={f}
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
            ...
            {plans.map((p, i) => (
              ...
              >
                ...
                <div className="p-7 flex flex-col flex-1">
                  ...
                  <div className="text-4xl font-bold text-gray-900 mb-5 dark:text-white">
                    {p.price}
                  </div>
                  <ul className="text-sm text-gray-600 space-y-2 mb-6 flex-1 dark:text-slate-300">
                    {p.features.map((f) => (
                      <li key={f} className="flex gap-2 items-start">
                        <span className="text-brand-600 mt-0.5 dark:text-brand-300">✔</span>
                        <span>{f}</span>
                      </li>
                    ))}
                  </ul>
          ...
          <aside className="relative rounded-3xl border border-white/50 bg-white/75 p-8 shadow-sm backdrop-blur dark:border-white/10 dark:bg-slate-900/60 dark:shadow-brand-950/40">
            ...
            <p className="mt-6 text-xs text-gray-500 dark:text-slate-400">
              ¿Necesitas algo específico? Escríbenos a <a href="mailto:team@filacero.com" className="font-medium text-brand-600 hover:underline dark:text-brand-200">team@filacero.com</a>.
            </p>
          </aside>
        </div>
      </div>

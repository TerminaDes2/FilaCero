...
export function Benefits() {
  return (
    <section id="benefits" aria-labelledby="benefits-heading" className="py-24 bg-white dark:bg-slate-950 relative">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <ul className="grid gap-6 md:grid-cols-3" aria-label="Lista de resultados">
          {benefits.map((b, idx) => (
            <li
              key={b.title}
              className="relative group rounded-2xl border border-gray-200/70 dark:border-white/10 bg-white/80 dark:bg-slate-900/50 backdrop-blur-sm overflow-hidden transition-shadow hover:shadow-lg focus-within:shadow-lg"
            >
              ...
              <div className="p-5 flex flex-col h-full">
                <div className="flex items-start gap-3 mb-2.5">
                  <span
                    aria-hidden
                    className="mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[var(--fc-brand-50)] to-[var(--fc-teal-50)] dark:from-slate-800/60 dark:to-slate-800/20 ring-1 ring-inset ring-gray-200/70 dark:ring-white/10 text-[11px] font-medium text-[var(--fc-brand-600)] dark:text-[var(--fc-brand-300)] group-hover:shadow-sm transition"
                  >
                    {idx + 1}
                  </span>
                  <h3 className="font-semibold tracking-tight text-sm text-gray-800 dark:text-slate-100 group-hover:text-[var(--fc-brand-600)] dark:group-hover:text-[var(--fc-brand-300)] transition-colors">
                    {b.title}
                  </h3>
                </div>
                <p className="text-[13px] leading-relaxed text-gray-600 dark:text-slate-300 flex-1">
                  {b.text}
                </p>
              </div>

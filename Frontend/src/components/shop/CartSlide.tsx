...
export default function CartSlide() {
  ...
  const itemsContainerClass = `px-4 sm:px-5 py-4 overflow-y-auto space-y-3 ${isMobile ? "max-h-[50vh]" : "max-h-[58vh]"}`;
  ...
  return (
    <>
      {/* Drawer for desktop */}
      <aside
        className={`hidden sm:block fixed inset-y-0 right-0 w-full sm:w-[420px] max-w-[92vw] bg-white/90 backdrop-blur-xl border-l border-[var(--fc-border-soft)] shadow-xl rounded-none sm:rounded-tl-3xl sm:rounded-bl-3xl transform transition-transform duration-300 z-40 ${open ? "translate-x-0" : "translate-x-full"}`}
        role="dialog"
        aria-label="Carrito"
        aria-modal="true"
      >
      ...
      {isMobile && (
        ...
        >
          <div className={`w-full max-w-md transform transition-all duration-300 ${open ? "translate-y-0 opacity-100" : "translate-y-6 opacity-0"}`}>
            <div className="overflow-hidden rounded-3xl border border-[var(--fc-border-soft)] bg-white shadow-[0_24px_55px_-28px_rgba(15,23,42,0.75)]">
              {header}
              {itemsContent}
              {footer}
            </div>

...
// A compact dropdown-style filter replacing the horizontal CategoryTabs.
// UX goals:
// - Light footprint next to SearchBox & ViewToggle.
// - Fast selection of category or 'Todas'.
// - Keyboard accessible.
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          ref={popRef}
          className="absolute z-40 mt-1 min-w-[160px] max-h-[260px] overflow-y-auto rounded-lg border bg-white shadow-lg p-1 flex flex-col text-xs"
          role="listbox"
          style={{ borderColor: 'var(--pos-card-border)' }}
        >
          {list.length === 0 && (
            <div className="px-2 py-2 text-[11px] text-gray-500">Sin categorías</div>
          )}
          {list.map(cat => {
            ...
            return (
              ...
              >
                {cat === 'all' ? 'Todas' : cat}
                {active && (
                  <svg viewBox="0 0 24 24" className="w-4 h-4 ml-auto" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12.5 10 17l9-10"/></svg>
                )}
              </button>
            );
          })}

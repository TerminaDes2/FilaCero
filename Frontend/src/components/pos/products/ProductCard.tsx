...
export const ProductCard = ({ product, view = "grid" }: ProductCardProps) => {
  ...
  const CandyBadge = ({
    ...
    return (
      <span
        className={`relative z-0 inline-flex items-center ${size === "sm" ? "px-2 py-0.5 text-[11px]" : "px-2.5 py-1 text-sm"} font-medium rounded-md ${className || ""}`}
      >
        <span
          aria-hidden
        ...
        />
        <span className="relative z-[1]" style={{ color: pal.text }}>
          {children}
        </span>
      </span>
    );
  ...
  return (
    <div className={`${base} ${view === "list" ? "flex gap-4 items-center" : ""}`} style={{ background: "var(--pos-card-bg)", borderColor: "var(--pos-card-border)" }}>
      {/* Imagen */}
      <div className={view === "grid" ? "relative w-full h-36 overflow-hidden" : "relative w-24 h-20 rounded-lg overflow-hidden flex-shrink-0"} style={{ background: "#f2e2c5" }}>
        ...
        {outOfStock && (
          <span className="absolute inset-0 bg-[rgba(255,255,255,0.78)] backdrop-blur-sm flex items-center justify-center text-[11px] font-semibold tracking-wide" style={{ color: "var(--pos-danger-text)" }}>
            Sin stock
          </span>
        )}
      </div>
      ...
      <div className={`${view === "grid" ? "p-3" : "flex-1 min-w-0"} relative z-0`}>
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-sm font-semibold leading-tight line-clamp-2" style={{ color: "var(--pos-text-heading)" }}>
            {product.name}
          </h3>
          {view === "grid" && <CandyBadge size="sm" variant="category">{product.category || "General"}</CandyBadge>}
        </div>

        {view === "list" && <CandyBadge size="sm" variant="category" className="mt-0.5">{product.category || "General"}</CandyBadge>}

        {product.description && view === "grid" && <p className="mt-1 text-[11px] line-clamp-2" style={{ color: "var(--pos-text-muted)" }}>{product.description}</p>}
      ...
      {showPanel && <AddToCartPanel product={product} onClose={() => setShowPanel(false)} />}
    </div>
  );
};

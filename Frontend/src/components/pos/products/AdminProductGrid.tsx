    ...
    return (
      <div className='h-[60vh] min-h-[340px] flex items-center justify-center'>
        <div className='text-center max-w-sm'>
          ...
          <p className='text-sm mb-3' style={{ color: 'var(--pos-text-heading)' }}>No encontramos productos que coincidan con tu búsqueda.</p>
        </div>
      </div>
    );
  }
        ...
        {editingStockId && (() => {
          ...
          return (
            <EditStockPanel
              ...
              inventory={inv ? { id: String(inv.id_inventario), cantidad_actual: Number(inv.cantidad_actual), stock_minimo: Number(inv.stock_minimo || 0) } : undefined}
              onClose={closeEditStock}
              onSaved={async () => { await load(); closeEditStock(); }}
            />
          );
        })()}
      </div>
  ...
  <div className='relative flex-1 min-h-0 overflow-y-auto pr-1 pb-4 custom-scroll-area'>
      ...
      <div className='sticky top-0 z-[5] mb-2'>
        <label className='w-full rounded-lg px-3 py-2 flex items-center gap-3 bg-[var(--pos-card-bg)] border border-[var(--pos-card-border)] cursor-pointer shadow-sm'>
          ...
          <span className='text-sm font-medium' style={{ color: 'var(--pos-text-heading)' }}>Seleccionar todos ({list.length})</span>
        </label>
      </div>
              ...
              <div className='flex items-start gap-2'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <div className='mt-2 flex items-center justify-between'>
                    ...
                    <span className='text-[11px] text-[var(--pos-text-muted)]'>Stock: {p.stock == null ? '—' : p.stock}</span>
                  </div>
                </div>
              </div>

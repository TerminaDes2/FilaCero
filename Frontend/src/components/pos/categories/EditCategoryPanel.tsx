...
import { useCategoriesStore, CategoryColor, CategoryItem } from '../../../pos/categoriesStore';
...
export const EditCategoryPanel: React.FC<EditCategoryPanelProps> = ({ category, onClose, onUpdated }) => {
  const { update, categories } = useCategoriesStore();
  ...
  const handleSubmit = async () => {
    ...
    const duplicate = categories.some(c => c.id !== category.id && c.name.toLowerCase() === n.toLowerCase());
    if (duplicate) {
    ...
    try {
      await update(category.id, {
        name: n !== category.name ? n : undefined,
        color: color !== category.color ? color : undefined,
        icon: icon !== (category.icon || '') ? (icon || undefined) : undefined,
      });
      onUpdated?.();
      onClose();
    } catch (e: unknown) {
